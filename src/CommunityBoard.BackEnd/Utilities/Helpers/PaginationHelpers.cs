using System.Collections.Generic;
using System.Linq;
using CommunityBoard.Core.Contracts.Requests.Queries;
using CommunityBoard.Core.DTOs.Responses;
using CommunityBoard.Core.Interfaces.Services;
using CommunityBoard.Core.Models.Filters;

namespace CommunityBoard.BackEnd.Utilities.Helpers
{
    public static class PaginationHelpers
    {
        //Pagination Video: https://www.youtube.com/watch?v=cKj6U4qDmgQ
        public static PagedResponse<T> CreatePaginatedResponse<T>(IUriService uriService, PaginationFilter pagination, List<T> response)
        {
            var nextPage = pagination.PageNumber >= 1
                ? uriService.GetAllAnnouncementsUri(
                    new PaginationQuery(pagination.PageNumber + 1, pagination.PageSize)).ToString()
                : null;
            
            var previousPage = pagination.PageNumber - 1 >= 1
                ? uriService.GetAllAnnouncementsUri(
                    new PaginationQuery(pagination.PageNumber - 1, pagination.PageSize)).ToString()
                : null;

            return new PagedResponse<T>
            {
                Data = response,
                PageNumber = pagination.PageNumber >= 1 ? pagination.PageNumber : (int?)null,
                PageSize = pagination.PageSize >= 1 ? pagination.PageSize : (int?)null,
                NextPage = response.Any() ? nextPage : null,
                PreviousPage = previousPage
            };
        }
    }
}